...
if (isset($_POST['createPerson'])) {
    ...
    if ($AddAs === 'employee') {
        ...
        $deptID = intval($_POST['departmentID']);
        $supervisorID = !empty($_POST['supervisorID']) ? intval($_POST['supervisorID']) : NULL;
        $employmentType = $_POST['employmentType'];

        if ($salary < 0 || !$deptID) die("Invalid salary or department ID.");

        $stmt = $conn->prepare("INSERT INTO EMPLOYEE (employeeID,hireDate,workShift,salary,deptID,supervisorID,employmentType)
                                VALUES (?,?,?,?,?,?,?)");
        $stmt->bind_param("issdiis", $personID, $hireDate, $workShift, $salary, $deptID, $supervisorID, $employmentType);
        $stmt->execute();
    }

    elseif ($AddAs === 'adopter') {
        $income = floatval($_POST['monthlyIncome']);
        $pets = intval($_POST['number_of_pets_owned']);
    ...
    elseif ($AddAs === 'vet') {
        $licenseNo = $_POST['vet_license_no'];
        $specialization = $_POST['specialization'];
        $experience = intval($_POST['years_of_experience']);
        $clinicID = intval($_POST['clinicID']);

        if (!$licenseNo || !$specialization || $experience < 0 || !$clinicID) die("Vet fields invalid.");
...
<form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
    ...
    <div id="employeeFields" class="hidden md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
            ...
            <select name="workShift" id="workShift" class="w-full border rounded p-2">
        ...
        <div>
            ...
            <select name="employmentType" id="employmentType" class="w-full border rounded p-2">
...
<script>
// Fix: add name attributes so data actually posts
